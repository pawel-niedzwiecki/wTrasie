...
export const LayoutWithTwoColumn: ComponentLayoutDataType = ({ siteBarPrimary, siteBarSecondary, dataFooter, seo, alert, children }) => {
  ...
  return (
    <>
      <NextSeo {...seo} />
      <Header ComponentAaboveHeader={alert && <WrapperAlert {...alert} />} res={res} tabs={[{ title: 'Home', value: '/', icon: <Home />, active: asPath === '/' }, { title: 'Usługi', value: '/s', icon: <Tool />, active: asPath === '/s' }]} callBack={(search: string) => setQuery(search)} />
